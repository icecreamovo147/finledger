...
async fn seed_record_with_image(db: &DbState) -> (i64, i64) {
    ...
    let record_id: i64 = sqlx::query_scalar(
        "INSERT INTO income_records (book_id, date, category, description, total_amount) \
         VALUES (?1, '2024-01-01', 'Print', '测试记录', 50000) RETURNING id",
    )
    .bind(book_id)
    .fetch_one(&pool)
    .await
...
async fn test_delete_record_db_first_then_files() {
    ...
    let record_count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM income_records WHERE id = ?1")
            .bind(record_id)
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    let image_count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM income_images WHERE id = ?1")
            .bind(image_id)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn test_delete_book_db_first_then_files() {
    ...
    let book_id: i64 =
        sqlx::query_scalar("SELECT book_id FROM income_records WHERE id = ?1")
            .bind(record_id)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn test_migration_idempotency() {
    ...
    let count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM schema_migrations")
            ...
            .await
            .unwrap();
    assert!(count.0 >= 8, "Expected at least 8 migration versions, got {}", count.0);
}
...
async fn test_integrity_check_after_migrations() {
    ...
    assert!(result.is_none(), "Integrity check should pass after migrations");
}
...
async fn test_maintenance_guard_auto_release() {
...

    {
        ...
        assert!(result.is_err(), "get_pool should fail while maintenance is held");
    }
    let result = db.get_pool().await;
    assert!(result.is_ok(), "get_pool should succeed after guard is dropped");
}
...
async fn test_full_backup_restore_with_images() {
    ...
    let pool = db.raw_pool().await;
    let book_id: i64 = sqlx::query_scalar("INSERT INTO account_books (name) VALUES ('BackupTest') RETURNING id")
        .fetch_one(&pool).await.unwrap();

    let record_id: i64 = sqlx::query_scalar(
        "INSERT INTO income_records (book_id, date, category, total_amount) VALUES (?1, '2024-01-01', 'Print', 10000) RETURNING id",
    ...
    let backup_path = backup::do_backup(&db, backup_dir.to_str().unwrap()).await.unwrap();
    ...
    assert!(images_dir.join("test.jpg").exists(), "Image file should be restored");
    assert_eq!(std::fs::read(images_dir.join("test.jpg")).unwrap(), b"fake image data");
    ...
    assert!(db.get_pool().await.is_ok(), "Maintenance lock should be released after restore");
}
...
async fn test_legacy_db_restore_releases_lock() {
    ...
    assert!(result.is_ok(), "Legacy restore should succeed: {:?}", result.err());

    assert!(db.get_pool().await.is_ok(), "Maintenance lock must be released after legacy .db restore");

    let msg = result.unwrap();
    assert!(msg.contains("旧版备份") || msg.contains("旧版"), "Should warn about old backup format, got: {}", msg);
}
...
async fn test_restore_failure_rollback_images() {
    ...
    let corrupt_backup = backup_dir.join("corrupt.flbackup");
    {
        ...
        zip.start_file("finledger.db", SimpleFileOptions::default()).unwrap();
    ...
    assert!(original_image.exists(), "Original images must survive a failed restore");
    assert_eq!(std::fs::read(&original_image).unwrap(), b"original data");
    assert!(db.get_pool().await.is_ok(), "DB should be usable after failed restore");
}
...
async fn test_restore_images_rollback_on_copy_failure() {
    ...
    let result = backup::do_restore(&db, &backup_path).await;
    assert!(result.is_err(), "Restore should fail when images_dir is a file");
    ...
    assert!(images_dir.exists(), "images path should exist after rollback");

    // DB should still be functional
    assert!(db.get_pool().await.is_ok(), "DB should be usable after failed restore");

    // No stale backup dirs
    assert!(!app_dir.join(".images-pre-restore").exists(), "Stale images backup should be cleaned up");
}
